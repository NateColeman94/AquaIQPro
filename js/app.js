...
function programDemand(){return state.programs.lessons.reduce((s,x)=>s+x.count,0)*.35+state.programs.parties.reduce((s,x)=>s+x.count,0)*.8+state.programs.team.reduce((s,x)=>s+x.lanes,0)*7}
...
function renderPrograms(){q("lessonList").innerHTML=state.programs.lessons.map((x,i)=>"<p><b>"+x.name+"</b><br>"+x.time+" • "+x.count+" swimmers <button onclick='removeProgram(\"lessons\","+i+")'>Remove</button></p>").join("");q("partyList").innerHTML=state.programs.parties.map((x,i)=>"<p><b>"+x.name+"</b><br>"+x.time+" • "+x.count+" guests <button onclick='removeProgram(\"parties\","+i+")'>Remove</button></p>").join("");q("teamList").innerHTML=state.programs.team.map((x,i)=>"<p><b>"+x.name+"</b><br>"+x.time+" • "+x.lanes+" lanes <button onclick='removeProgram(\"team\","+i+")'>Remove</button></p>").join("");var lanes=state.programs.team.reduce((s,x)=>s+x.lanes,0);q("lanePressure").innerHTML="Current reserved swim team lanes: <b>"+lanes+"</b>. "+(lanes>=5?"Pressure is high.":"Pressure is manageable.")}
...
function addIncident(){if(q("incidentInput").value.trim()){state.incidents.unshift(q("incidentInput").value.trim());q("incidentInput").value="";updateInputs("Incident added")}}function addStaff(){if(q("newStaffName").value.trim()){state.staff.push({name:q("newStaffName").value,shift:q("newStaffShift").value||"TBD",area:q("newStaffArea").value,status:"Scheduled"});q("newStaffName").value="";q("newStaffShift").value="";updateInputs("Staff member added")}}function removeStaff(i){state.staff.splice(i,1);updateInputs("Staff member removed")}function addProgram(type){if(type==="lessons")state.programs.lessons.push({name:"New Lesson Block",time:"2:00 PM",count:12});if(type==="parties")state.programs.parties.push({name:"New Pool Party",time:"3:00 PM",count:20});if(type==="team")state.programs.team.push({name:"Swim Team Practice",time:"5:00 PM",lanes:4});updateInputs("Program schedule updated")}function removeProgram(type,i){state.programs[type].splice(i,1);updateInputs("Program removed")}function addWorkOrder(){if(q("woAsset").value.trim()){state.workOrders.unshift({asset:q("woAsset").value.trim(),priority:q("woPriority").value,status:q("woStatus").value,desc:q("woDesc").value.trim()||"No description entered."});q("woAsset").value="";q("woDesc").value="";updateInputs("Work order added")}}function addTask(){if(q("taskName").value.trim()){state.tasks.unshift({name:q("taskName").value.trim(),priority:q("taskPriority").value,owner:q("taskOwner").value,done:false});q("taskName").value="";updateInputs("Daily task added")}}function toggleTask(i){state.tasks[i].done=!state.tasks[i].done;updateInputs("Daily task status changed")}function decide(id,status){state.decisions[id]=status;state.audit.unshift(new Date().toLocaleString()+": Recommendation "+id+" "+status);render();saveState(false)}
