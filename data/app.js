console.info("AquaIQPro v5.7.4 Dashboard & UX Polish loaded");
...
function persistCompleteFacilityState(){
  ...
  if(!state.operationsByFacility)return;
  var current=state.operationsByFacility[activeFacilityId]||{};
  state.operationsByFacility[activeFacilityId]=Object.assign({},current,{
    water:state.water,callouts:state.callouts,inventory:state.inventory,
    workOrders:state.workOrders,tasks:state.tasks,incidents:state.incidents,decisions:state.decisions,
    waterHistory:state.waterHistory,waterSchedule:state.waterSchedule,
    cleaningTasks:state.cleaningTasks,inspections:state.inspections,
    preventiveMaintenance:state.preventiveMaintenance,maintenanceHistory:state.maintenanceHistory
  });
}
...
/* AquaIQPro v5.7.5 Operational Modules */
function today57D(){return new Date().toISOString().slice(0,10)}
function now57D(){return new Date().toLocaleString()}
function esc57D(v){return String(v==null?"":v).replace(/[&<>"']/g,function(ch){return({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[ch]})}
function waterCheckStatus57D(x){if(!x)return"Unknown";if(x.chlorine<1||x.chlorine>4||x.ph<7.2||x.ph>7.8||x.alk<80||x.alk>120)return"Action";if(x.chlorine<1.5||x.chlorine>3.5||x.ph<7.3||x.ph>7.7||x.alk<85||x.alk>115)return"Watch";return"Good"}
function inferInventoryCategory57D(item){var n=(item.item||"").toLowerCase();if(/chlorine|acid|reagent|chemical/.test(n))return"Chemicals";if(/clean|soap|sanit|deck supplies/.test(n))return"Cleaning Supplies";if(/first aid|bandage|aed|ice pack/.test(n))return"First Aid";if(/rescue|tube|rope|backboard|whistle/.test(n))return"Rescue Equipment";if(/ppe|glove|mask|goggle/.test(n))return"PPE";return item.category||"Other"}
function ensureSprint57DState(){
  if(!state.operationsByFacility)return;
  var ops=state.operationsByFacility[activeFacilityId];
  if(!ops.waterHistory)ops.waterHistory=[{timestamp:now57D(),recordedBy:"System baseline",location:"Main Pool",chlorine:ops.water.chlorine,ph:ops.water.ph,alk:ops.water.alk,temp:82,notes:"Baseline reading"}];
  if(!ops.waterSchedule)ops.waterSchedule=[{time:"08:00",location:"Main Pool",done:false},{time:"12:00",location:"Main Pool",done:false},{time:"16:00",location:"Main Pool",done:false}];
  if(!ops.cleaningTasks)ops.cleaningTasks=[{name:"Disinfect high-touch deck surfaces",area:"Pool Deck",frequency:"Opening",done:false},{name:"Inspect and clean bathrooms",area:"Bathrooms",frequency:"Midday",done:false},{name:"Closing locker-room sanitation",area:"Locker Rooms",frequency:"Closing",done:false}];
  if(!ops.inspections)ops.inspections=[{asset:"Rescue equipment",type:"Safety",status:"Pass",notes:"Opening inspection",timestamp:now57D()}];
  if(!ops.preventiveMaintenance)ops.preventiveMaintenance=[{asset:"Main circulation pump",due:today57D(),frequency:"Monthly",status:"Due"},{asset:"Emergency phone",due:today57D(),frequency:"Weekly",status:"Due"}];
  if(!ops.maintenanceHistory)ops.maintenanceHistory=[];
  ops.inventory.forEach(function(i){if(!i.category)i.category=inferInventoryCategory57D(i)});
  state.waterHistory=ops.waterHistory;state.waterSchedule=ops.waterSchedule;state.cleaningTasks=ops.cleaningTasks;state.inspections=ops.inspections;state.preventiveMaintenance=ops.preventiveMaintenance;state.maintenanceHistory=ops.maintenanceHistory;
  if(typeof state.water.temp!=="number")state.water.temp=(ops.waterHistory[0]&&Number(ops.waterHistory[0].temp))||82;
}
function saveWaterCheck(recordedBy){ensureSprint57DState();state.water.chlorine=Number(q("chlorineInput").value||state.water.chlorine);state.water.ph=Number(q("phInput").value||state.water.ph);state.water.alk=Number(q("alkInput").value||state.water.alk);state.water.temp=Number(q("waterTempInput").value||state.water.temp||82);state.waterHistory.unshift({timestamp:now57D(),recordedBy:recordedBy||"Manager",location:q("waterLocationInput").value,chlorine:state.water.chlorine,ph:state.water.ph,alk:state.water.alk,temp:state.water.temp,notes:""});state.audit.unshift(now57D()+": Water check saved by "+(recordedBy||"Manager"));render();saveState(false)}
function saveLifeguardWaterCheck(){ensureSprint57DState();var guard=q("waterGuardInput").value;if(!guard)return alert("Select a lifeguard.");var x={timestamp:now57D(),recordedBy:guard,location:q("waterGuardLocation").value,chlorine:Number(q("waterGuardChlorine").value),ph:Number(q("waterGuardPh").value),alk:Number(q("waterGuardAlk").value),temp:Number(q("waterGuardTemp").value),notes:q("waterGuardNotes").value};if(!x.chlorine||!x.ph||!x.alk)return alert("Enter chlorine, pH, and alkalinity.");state.waterHistory.unshift(x);state.water={chlorine:x.chlorine,ph:x.ph,alk:x.alk,temp:x.temp||state.water.temp};state.audit.unshift(now57D()+": Lifeguard water check submitted by "+guard);q("waterGuardNotes").value="";render();saveState(false)}
function addWaterTestSchedule(){ensureSprint57DState();var t=q("waterScheduleTime").value;if(!t)return alert("Choose a test time.");state.waterSchedule.push({time:t,location:q("waterScheduleLocation").value,done:false});render();saveState(false)}
function toggleWaterSchedule57D(i){state.waterSchedule[i].done=!state.waterSchedule[i].done;render();saveState(false)}
function removeWaterSchedule57D(i){state.waterSchedule.splice(i,1);render();saveState(false)}
function renderWater(){ensureSprint57DState();var rows=[["Free Chlorine","1.0–4.0 ppm",state.water.chlorine,state.water.chlorine>=1&&state.water.chlorine<=4?"Low":"High"],["pH","7.2–7.8",state.water.ph,state.water.ph>=7.2&&state.water.ph<=7.8?"Low":"High"],["Alkalinity","80–120 ppm",state.water.alk,state.water.alk>=80&&state.water.alk<=120?"Low":"Medium"],["Water Temperature","Facility target",(state.water.temp||82)+"°F","Low"]];q("chemicalRanges").innerHTML=rows.map(function(r){return"<tr><td>"+r[0]+"</td><td>"+r[1]+"</td><td>"+r[2]+"</td><td>"+tag(r[3])+"</td></tr>"}).join("");q("waterTempInput").value=state.water.temp||82;var guards=q("waterGuardInput");if(guards)guards.innerHTML='<option value="">Select lifeguard</option>'+state.staff.map(function(s){return'<option>'+esc57D(s.name)+'</option>'}).join("");q("waterTestingSchedule").innerHTML=state.waterSchedule.map(function(x,i){return'<div class="rowbox rowflex '+(x.done?'done':'')+'"><div><b>'+esc57D(x.time)+'</b><br><span class="small">'+esc57D(x.location)+'</span></div><div class="actions"><button class="btn '+(x.done?'secondary':'good')+'" onclick="toggleWaterSchedule57D('+i+')">'+(x.done?'Undo':'Complete')+'</button><button class="btn bad" onclick="removeWaterSchedule57D('+i+')">Remove</button></div></div>'}).join("");var current={chlorine:state.water.chlorine,ph:state.water.ph,alk:state.water.alk};var status=waterCheckStatus57D(current);var alerts=[];if(status==="Action")alerts.push(tag("High")+" Reading outside target range. Restrict use if required by local policy, correct chemistry, and retest.");else if(status==="Watch")alerts.push(tag("Medium")+" Reading is near the edge of the target range. Increase testing frequency.");else alerts.push(tag("Low")+" Current chemistry is within configured target ranges.");var missed=state.waterSchedule.filter(function(x){return !x.done}).length;if(missed)alerts.push(tag("Medium")+" "+missed+" scheduled test(s) remain incomplete.");q("waterAlerts").innerHTML=alerts.map(function(x){return'<p>'+x+'</p>'}).join("");var wr=recommendations().filter(function(r){return r.id.indexOf("water")>=0});q("waterGuidance").innerHTML=wr.length?wr.map(function(r){return"<p>"+tag(r.p)+" "+r.t+"<br><span class='small'>"+r.why+"</span></p>"}).join(""):"<p>Water readings are within target range. Continue scheduled testing and document each check.</p>";var hist=state.waterHistory.slice(0,30);q("waterHistoryTable").innerHTML=hist.map(function(x){return'<tr><td>'+esc57D(x.timestamp)+'</td><td>'+esc57D(x.recordedBy)+'</td><td>'+esc57D(x.location)+'</td><td>'+x.chlorine+'</td><td>'+x.ph+'</td><td>'+x.alk+'</td><td>'+(x.temp||'--')+'</td><td>'+tag(waterCheckStatus57D(x)==="Good"?"Low":waterCheckStatus57D(x)==="Watch"?"Medium":"High")+' '+waterCheckStatus57D(x)+'</td></tr>'}).join("");var avg=function(k){return hist.length?(hist.reduce(function(s,x){return s+Number(x[k]||0)},0)/hist.length).toFixed(1):"--"};q("waterTrendMetrics").innerHTML=[['Checks recorded',hist.length,'Recent history'],['Average chlorine',avg('chlorine')+' ppm','Recorded checks'],['Average pH',avg('ph'),'Recorded checks'],['Average temp',avg('temp')+'°F','Recorded checks']].map(function(m){return'<div class="metric-card"><span>'+m[0]+'</span><b>'+m[1]+'</b><small>'+m[2]+'</small></div>'}).join("")}
function inventoryRecommendation57D(i){var calc=inventoryCalculated().find(function(x){return x.item===i.item})||i;if(i.onHand<=i.min)return"Order now — below minimum.";if(calc.days<=7)return"Order within 48 hours — "+calc.days+" days projected.";return"Monitor — no immediate purchase needed."}
function updateInventory57D(i,field,value){state.inventory[i][field]=field==="category"?value:Number(value);render();saveState(false)}
function removeInventory57D(i){if(confirm("Remove this inventory item?")){state.inventory.splice(i,1);render();saveState(false)}}
function addInventoryItem57D(){var name=q("inventoryItemName").value.trim();if(!name)return alert("Enter an item name.");state.inventory.push({item:name,category:q("inventoryItemCategory").value,unit:q("inventoryItemUnit").value.trim()||"units",onHand:Number(q("inventoryItemOnHand").value||0),min:Number(q("inventoryItemMinimum").value||0),dailyUse:Number(q("inventoryItemDailyUse").value||.1)});["inventoryItemName","inventoryItemUnit","inventoryItemOnHand","inventoryItemMinimum","inventoryItemDailyUse"].forEach(function(id){q(id).value=""});render();saveState(false)}
function renderInventory(){ensureSprint57DState();var cats=["Chemicals","Cleaning Supplies","First Aid","Rescue Equipment","PPE","Other"];var calcs=inventoryCalculated();var flagged=calcs.filter(function(i){return i.onHand<=i.min||i.days<=7});q("inventorySummaryMetrics").innerHTML=[['Inventory items',state.inventory.length,'Across all categories'],['Low-stock flags',flagged.length,'Require review'],['Below minimum',state.inventory.filter(function(i){return i.onHand<=i.min}).length,'Order now'],['Categories used',new Set(state.inventory.map(function(i){return i.category})).size,'Operational groups']].map(function(m){return'<div class="metric-card"><span>'+m[0]+'</span><b>'+m[1]+'</b><small>'+m[2]+'</small></div>'}).join("");q("inventoryCategoryPanels").innerHTML=cats.map(function(cat){var items=state.inventory.map(function(x,i){return{x:x,i:i}}).filter(function(o){return(o.x.category||inferInventoryCategory57D(o.x))===cat});if(!items.length)return"";return'<details class="module-panel" open><summary>'+cat+' <span class="small">('+items.length+')</span></summary><div class="module-panel-body"><div class="table-wrap"><table><thead><tr><th>Item</th><th>On Hand</th><th>Minimum</th><th>Days Left</th><th>Recommendation</th><th></th></tr></thead><tbody>'+items.map(function(o){var i=o.i,x=o.x,calc=calcs.find(function(y){return y.item===x.item})||x;return'<tr><td><b>'+esc57D(x.item)+'</b><br><span class="small">'+esc57D(x.unit)+'</span></td><td><input class="table-input" type="number" step=".1" value="'+x.onHand+'" onchange="updateInventory57D('+i+',\'onHand\',this.value)"></td><td><input class="table-input" type="number" step=".1" value="'+x.min+'" onchange="updateInventory57D('+i+',\'min\',this.value)"></td><td>'+calc.days+'</td><td>'+inventoryRecommendation57D(x)+'</td><td><button class="btn bad" onclick="removeInventory57D('+i+')">Remove</button></td></tr>'}).join("")+'</tbody></table></div></div></details>'}).join("");q("inventoryRecommendations57D").innerHTML=flagged.length?flagged.map(function(i){return'<div class="rowbox"><b>'+esc57D(i.item)+'</b> '+tag(i.onHand<=i.min?"High":"Medium")+'<p>'+inventoryRecommendation57D(i)+'</p></div>'}).join(""):'<p>'+tag("Low")+' No immediate purchase recommendations.</p>'}
function addCleaningTask57D(){var name=q("cleaningTaskName").value.trim();if(!name)return alert("Enter a cleaning task.");state.cleaningTasks.push({name:name,area:q("cleaningTaskArea").value,frequency:q("cleaningTaskFrequency").value,done:false});q("cleaningTaskName").value="";render();saveState(false)}
function toggleCleaning57D(i){state.cleaningTasks[i].done=!state.cleaningTasks[i].done;if(state.cleaningTasks[i].done)state.maintenanceHistory.unshift({timestamp:now57D(),type:"Cleaning",item:state.cleaningTasks[i].name,outcome:"Completed"});render();saveState(false)}
function removeCleaning57D(i){state.cleaningTasks.splice(i,1);render();saveState(false)}
function addInspection57D(){var asset=q("inspectionAsset57D").value.trim();if(!asset)return alert("Enter an area or equipment item.");var x={asset:asset,type:q("inspectionType57D").value,status:q("inspectionStatus57D").value,notes:q("inspectionNotes57D").value,timestamp:now57D()};state.inspections.unshift(x);state.maintenanceHistory.unshift({timestamp:x.timestamp,type:x.type+" inspection",item:x.asset,outcome:x.status});q("inspectionAsset57D").value="";q("inspectionNotes57D").value="";render();saveState(false)}
function addPreventive57D(){var asset=q("preventiveAsset57D").value.trim(),due=q("preventiveDue57D").value;if(!asset||!due)return alert("Enter an asset and due date.");state.preventiveMaintenance.push({asset:asset,due:due,frequency:q("preventiveFrequency57D").value,status:"Scheduled"});q("preventiveAsset57D").value="";q("preventiveDue57D").value="";render();saveState(false)}
function completePreventive57D(i){var x=state.preventiveMaintenance[i];x.status="Completed";state.maintenanceHistory.unshift({timestamp:now57D(),type:"Preventive maintenance",item:x.asset,outcome:"Completed"});render();saveState(false)}
function renderMaintenance(){ensureSprint57DState();q("workOrders").innerHTML=state.workOrders.map(function(w,i){return"<div class='rowbox'><b>"+esc57D(w.asset)+"</b> "+tag(w.priority)+"<p>"+esc57D(w.desc)+"</p><select onchange='state.workOrders["+i+"].status=this.value;if(this.value===\"Completed\")state.maintenanceHistory.unshift({timestamp:now57D(),type:\"Work order\",item:state.workOrders["+i+"].asset,outcome:\"Completed\"});updateInputs(\"Work order status changed\")'><option "+(w.status==="Open"?"selected":"")+">Open</option><option "+(w.status==="In Progress"?"selected":"")+">In Progress</option><option "+(w.status==="Completed"?"selected":"")+">Completed</option></select></div>"}).join("");q("maintenanceAI").innerHTML=highOpenWorkOrders().length?highOpenWorkOrders().map(function(w){return"<p>"+tag("High")+" "+esc57D(w.asset)+": "+esc57D(w.desc)+"</p>"}).join(""):"<p>"+tag("Low")+" No high-priority maintenance blockers.</p>";q("cleaningTaskList").innerHTML=state.cleaningTasks.map(function(x,i){return'<div class="rowbox rowflex '+(x.done?'done':'')+'"><div><b>'+esc57D(x.name)+'</b><br><span class="small">'+esc57D(x.area)+' • '+esc57D(x.frequency)+'</span></div><div class="actions"><button class="btn '+(x.done?'secondary':'good')+'" onclick="toggleCleaning57D('+i+')">'+(x.done?'Undo':'Complete')+'</button><button class="btn bad" onclick="removeCleaning57D('+i+')">Remove</button></div></div>'}).join("");q("inspectionList57D").innerHTML=state.inspections.map(function(x){return'<div class="rowbox"><b>'+esc57D(x.asset)+'</b> '+tag(x.status==="Pass"?"Low":x.status==="Needs Attention"?"Medium":"High")+'<p class="small">'+esc57D(x.type)+' • '+esc57D(x.timestamp)+'</p><p>'+esc57D(x.notes||"No notes")+'</p></div>'}).join("");q("preventiveList57D").innerHTML=state.preventiveMaintenance.map(function(x,i){var overdue=x.status!=="Completed"&&x.due<today57D();return'<div class="rowbox rowflex"><div><b>'+esc57D(x.asset)+'</b> '+tag(x.status==="Completed"?"Low":overdue?"High":"Medium")+'<br><span class="small">Due '+esc57D(x.due)+' • '+esc57D(x.frequency)+' • '+esc57D(x.status)+'</span></div>'+(x.status!=="Completed"?'<button class="btn good" onclick="completePreventive57D('+i+')">Complete</button>':'')+'</div>'}).join("");q("maintenanceHistory57D").innerHTML=state.maintenanceHistory.length?state.maintenanceHistory.slice(0,40).map(function(x){return'<div class="rowbox"><b>'+esc57D(x.item)+'</b><br><span class="small">'+esc57D(x.type)+' • '+esc57D(x.timestamp)+' • '+esc57D(x.outcome)+'</span></div>'}).join(""):'<p class="small">Completed cleaning, inspections, preventive items, and work orders will appear here.</p>';var open=state.workOrders.filter(function(x){return x.status!=="Completed"}).length,cleanDone=state.cleaningTasks.filter(function(x){return x.done}).length,attention=state.inspections.filter(function(x){return x.status!=="Pass"}).length,preventiveDue=state.preventiveMaintenance.filter(function(x){return x.status!=="Completed"&&x.due<=today57D()}).length;q("maintenanceSummaryMetrics").innerHTML=[['Open work orders',open,'Current queue'],['Cleaning complete',cleanDone+'/'+state.cleaningTasks.length,'Today'],['Inspection issues',attention,'Need attention'],['Preventive due',preventiveDue,'Due or overdue']].map(function(m){return'<div class="metric-card"><span>'+m[0]+'</span><b>'+m[1]+'</b><small>'+m[2]+'</small></div>'}).join("")}

/* AquaIQPro v5.7.8 — final architecture, role integration, and dark mode */
(function(){
  "use strict";
  ...
  function el(id){return document.getElementById(id)}
  function safeNum(v,d){v=Number(v);return isFinite(v)?v:(d||0)}
  function currentOps(){try{return state.operationsByFacility&&state.operationsByFacility[activeFacilityId]||state}catch(e){return {}}}
  function facilityName(){try{return activeFacility().name}catch(e){return (el("facilitySelect")&&el("facilitySelect").selectedOptions[0].text)||"Pool"}}
  function activeStaff(){try{return (state.staff||[]).filter(function(s){return s.status!=="Unavailable"&&s.status!=="Off"})}catch(e){return []}}
  function waterStatus(){try{var w=state.water||{};return w.chlorine>=1&&w.chlorine<=4&&w.ph>=7.2&&w.ph<=7.8&&w.alk>=80&&w.alk<=120?"GOOD":"CHECK"}catch(e){return"--"}}
  function forecast(){try{return safeNum(state.forecast||state.demand||state.expectedAttendance,0)}catch(e){return 0}}
  function capacity(){try{return safeNum(state.capacity||state.operatingCapacity||((state.staff||[]).length*25),150)}catch(e){return 150}}
  function renderModes(){
    var f=facilityName(), staff=activeStaff(), cap=capacity(), demand=forecast(), pct=Math.min(100,Math.round((demand/Math.max(1,cap))*100)), ws=waterStatus(), ops=currentOps(), programs=(state.programs||ops.programs||[]);
    ["deckFacility","lobbyFacility"].forEach(function(id){if(el(id))el(id).textContent=f});
    var now=new Date(), tm=now.toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}), dt=now.toLocaleDateString([],{weekday:"long",month:"long",day:"numeric"});
    ["lgClock","deckClock","lobbyClock"].forEach(function(id){if(el(id))el(id).textContent=tm});["lgDate","deckDate","lobbyDate"].forEach(function(id){if(el(id))el(id).textContent=dt});
    if(el("lgMetrics"))el("lgMetrics").innerHTML=[["Scheduled Staff",staff.length,"On duty / available"],["Expected Attendance",demand||"--","Today’s forecast"],["Operating Capacity",cap,"Current safe limit"],["Water Status",ws,"Latest chemical readings"]].map(function(x){return'<div class="metric-card"><span>'+x[0]+'</span><b>'+x[1]+'</b><small>'+x[2]+'</small></div>'}).join("");
    if(el("lgAssignment")){var names=staff.slice(0,5);el("lgAssignment").innerHTML=names.length?names.map(function(s,i){return'<div class="rowbox"><b>'+s.name+'</b><br><span class="small">'+(s.assignment||s.position||(["Main Pool","Shallow Area","Relief","Break Rotation"][i%4]))+'</span></div>'}).join(""):'<p class="small">No active assignments are available.</p>'}
    if(el("lgNotices")){var notices=[];if(ws!=="GOOD")notices.push("Water readings require review before normal operations continue.");if(demand>cap*.85)notices.push("High attendance is expected. Closely monitor swimmer count and rotations.");if(!notices.length)notices.push("No critical safety alerts. Continue scheduled checks and rotation procedures.");el("lgNotices").innerHTML=notices.map(function(n){return'<div class="rowbox">'+n+'</div>'}).join("")}
    if(el("lgCredentials")){var person=(state.staff||[])[0]||{};el("lgCredentials").innerHTML='<div class="rowbox"><b>'+(person.name||"On-duty lifeguard")+'</b><p class="small">Lifeguard certification: Active<br>CPR/AED: Active<br>Next renewal review: 30-day alerts enabled</p></div>'}
    if(el("deckOccupancy"))el("deckOccupancy").textContent=demand||"--";if(el("deckCapacityNote"))el("deckCapacityNote").textContent="of "+cap+" capacity ("+pct+"%)";if(el("deckOccupancyBar"))el("deckOccupancyBar").style.width=pct+"%";
    if(el("deckStaffing"))el("deckStaffing").textContent=staff.length+" on duty";if(el("deckStaffingNote"))el("deckStaffingNote").textContent=Math.floor(staff.length*25)+" swimmers supported";
    if(el("deckWater"))el("deckWater").textContent=ws;if(el("deckWaterNote"))el("deckWaterNote").textContent=ws==="GOOD"?"Within target ranges":"Manager review required";
    if(el("deckTemp"))el("deckTemp").textContent=safeNum(state.water&&state.water.temp,82)+"°F";
    if(el("deckRotations"))el("deckRotations").innerHTML=staff.slice(0,5).map(function(s,i){var assignment=s.assignment||["Main Pool → Relief","Shallow Area → Break","Relief → Main Pool","Break → Shallow Area"][i%4];return'<div class="rotation-command-row"><b>'+(s.name||"Guard "+(i+1))+'</b><span>'+assignment+'</span><em>'+(i===0?"Now":"Next cycle")+'</em></div>'}).join("")||'<p class="small">Rotation plan not yet published.</p>';
    if(el("deckWeather"))el("deckWeather").innerHTML='<div class="display-tile" style="min-height:135px;box-shadow:none;margin-top:12px"><span>Current Conditions</span><b>'+(state.weather&&state.weather.temp?state.weather.temp+"°F":"Live weather")+'</b><small>'+(state.weather&&state.weather.summary||"Monitor heat, lightning, and rain alerts.")+'</small></div>';if(el("deckSafetyNotes")){var safety=[];if(ws!=="GOOD")safety.push("Chemical readings require manager review.");if(pct>=85)safety.push("Pool use is above 85%; increase headcounts and rotation awareness.");if(staff.length<Math.ceil((demand||0)/25))safety.push("Staffing may not support forecast demand at the 1:25 planning ratio.");if(!safety.length)safety.push("No active deck safety exceptions.");el("deckSafetyNotes").innerHTML=safety.map(function(x){return'<div>'+x+'</div>'}).join("")}
    var status=(ws==="GOOD"&&staff.length>0)?"OPEN":"CHECK STATUS";["deckOpenStatus","lobbyOpenStatus"].forEach(function(id){var x=el(id);if(x){x.textContent=id==="lobbyOpenStatus"?(status==="OPEN"?"WE’RE OPEN":"PLEASE CHECK WITH STAFF"):status;x.classList.toggle("warning",status!=="OPEN")}});if(el("lobbyPublicStatus"))el("lobbyPublicStatus").textContent=status==="OPEN"?"Open":"Check with staff";if(el("deckAlertStrip")){var da=el("deckAlertStrip"),msg=ws!=="GOOD"?"Water readings require review before normal operation.":pct>=85?"High pool use — increase headcounts and maintain rotation discipline.":staff.length<Math.ceil((demand||0)/25)?"Staffing coverage is below forecast need.":"Operations normal — continue scheduled rotations and safety checks.";da.className="display-alert-strip"+(ws!=="GOOD"?" critical":pct>=85||staff.length<Math.ceil((demand||0)/25)?" warning":"");da.innerHTML='<b>'+(ws!=="GOOD"?"Action required":pct>=85?"High use period":"Operations normal")+'</b><span>'+msg+'</span>'}
    if(el("lobbyOccupancy"))el("lobbyOccupancy").textContent=pct+"%";if(el("lobbyOccupancyBar"))el("lobbyOccupancyBar").style.width=pct+"%";if(el("lobbyOccupancyNote"))el("lobbyOccupancyNote").textContent=(demand||0)+" expected swimmers • "+cap+" operating capacity";if(el("lobbyComfortNote")){el("lobbyComfortNote").textContent=pct<55?"Plenty of room available":pct<80?"Moderate pool use":"Busy pool period"}
    if(el("lobbyTemp"))el("lobbyTemp").textContent=safeNum(state.water&&state.water.temp,82)+"°F";if(el("lobbyWeather"))el("lobbyWeather").textContent=(state.weather&&state.weather.temp?state.weather.temp+"°F":"Pleasant");
    if(el("lobbyPrograms"))el("lobbyPrograms").innerHTML=(programs.length?programs.slice(0,5).map(function(p){return'<div class="lobby-program-item"><div><b>'+(p.name||p.title||"Aquatics Program")+'</b><span>'+(p.location||p.area||"Pool area")+'</span></div><time>'+(p.time||p.start||"Today")+'</time></div>'}).join(""):'<div class="lobby-program-item"><div><b>Open Swim</b><span>See front desk for today’s schedule</span></div><time>Today</time></div>');
  }
  function setMode(mode){
    if(!permissions[mode])mode="manager";localStorage.setItem(MODE_KEY,mode);document.body.classList.remove("role-manager","role-lifeguard","role-deck","role-lobby");document.body.classList.add("role-"+mode);if(el("roleModeSelect"))el("roleModeSelect").value=mode;if(el("modeSubtitle"))el("modeSubtitle").textContent=subtitles[mode];
    document.querySelectorAll("#nav button[data-page]").forEach(function(btn){var allowed=permissions[mode].indexOf(btn.dataset.page)>=0;btn.classList.toggle("role-hidden",!allowed)});document.querySelectorAll("#nav details.nav-group").forEach(function(group){var visible=group.querySelector("button[data-page]:not(.role-hidden)");group.classList.toggle("role-hidden",!visible)});
    var target={manager:"dashboard",lifeguard:"lifeguardmode",deck:"deckmode",lobby:"lobbymode"}[mode];renderModes();if(typeof showPage==="function")showPage(target);if(el("pageTitle"))el("pageTitle").textContent={manager:"Dashboard",lifeguard:"Lifeguard Shift",deck:"Deck Operations",lobby:"Lobby Display"}[mode];
  }
  function applyTheme(theme){if(theme==="system")theme=window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light";document.documentElement.dataset.theme=theme;localStorage.setItem(THEME_KEY,theme);if(el("themeToggle")){el("themeToggle").textContent=theme==="dark"?"☀️":"🌙";el("themeToggle").title=theme==="dark"?"Use light mode":"Use dark mode"}}
  function init57F(){
    applyTheme(localStorage.getItem(THEME_KEY)||"light");if(el("themeToggle"))el("themeToggle").addEventListener("click",function(){applyTheme(document.documentElement.dataset.theme==="dark"?"light":"dark")});if(el("roleModeSelect"))el("roleModeSelect").addEventListener("change",function(){setMode(this.value)});
    document.querySelectorAll("[data-role-target]").forEach(function(btn){btn.addEventListener("click",function(){var page=this.dataset.roleTarget, focus=this.dataset.roleFocus;if(typeof showPage==="function")showPage(page);if(focus)setTimeout(function(){var x=el(focus);if(x){if(x.tagName.toLowerCase()==="details")x.open=true;x.scrollIntoView({behavior:"smooth",block:"start"})}},80)})});
