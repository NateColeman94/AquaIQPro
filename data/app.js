...
/* AquaIQPro v5.7.7 — role-based operations and dark mode */
(function(){
  "use strict";
  ...
  function renderModes(){
    ...
    if(el("deckOccupancy"))el("deckOccupancy").textContent=demand||"--";if(el("deckCapacityNote"))el("deckCapacityNote").textContent="of "+cap+" capacity ("+pct+"%)";
    ...
    if(el("deckRotations"))el("deckRotations").innerHTML=staff.slice(0,4).map(function(s,i){return'<div class="trend-summary-row"><b>'+(s.name||"Guard "+(i+1))+'</b><span>'+(s.assignment||["Main Pool → Relief","Shallow Area → Break","Relief → Main Pool","Break → Shallow Area"][i%4])+'</span></div>'}).join("")||'<p class="small">Rotation plan not yet published.</p>';
    if(el("deckWeather"))el("deckWeather").innerHTML='<div class="display-tile" style="min-height:150px"><span>Conditions</span><b>'+(state.weather&&state.weather.temp?state.weather.temp+"°F":"Live weather")+'</b><small>'+(state.weather&&state.weather.summary||"Monitor heat, lightning, and rain alerts.")+'</small></div>';
    var status=(ws==="GOOD"&&staff.length>0)?"OPEN":"CHECK STATUS";["deckOpenStatus","lobbyOpenStatus"].forEach(function(id){var x=el(id);if(x){x.textContent=id==="lobbyOpenStatus"?(status==="OPEN"?"WE’RE OPEN":"PLEASE CHECK WITH STAFF"):status;x.classList.toggle("warning",status!=="OPEN")}});
    if(el("lobbyOccupancy"))el("lobbyOccupancy").textContent=pct+"%";if(el("lobbyOccupancyBar"))el("lobbyOccupancyBar").style.width=pct+"%";if(el("lobbyOccupancyNote"))el("lobbyOccupancyNote").textContent=(demand||0)+" expected swimmers • "+cap+" operating capacity";
    ...
    if(el("lobbyPrograms"))el("lobbyPrograms").innerHTML=(programs.length?programs.slice(0,4).map(function(p){return'<p><b>'+(p.name||p.title||"Aquatics Program")+'</b><br><span class="small">'+(p.time||p.start||"Today")+'</span></p>'}).join(""):'<p>Open Swim<br><span class="small">See front desk for today’s schedule</span></p>');
  }
