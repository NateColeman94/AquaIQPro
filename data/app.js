...
function ensureCompleteFacilityState(){
  ...
  enforceGandyFullNamesV561();
  ensureGandyFullNames();
...
function ensureGandyFullNames(){
  ...
  function upgrade(roster){
    if(!Array.isArray(roster))return;
    roster.forEach(function(guard){
      if(!guard||typeof guard.name!=="string")return;
      var trimmed=guard.name.trim();
      if(fullNameMap[trimmed])guard.name=fullNameMap[trimmed];
    });
  }

  if(state.staffByFacility&&state.staffByFacility.gandy){
    upgrade(state.staffByFacility.gandy);
  }
  if(activeFacilityId==="gandy"&&Array.isArray(state.staff)){
    upgrade(state.staff);
  }
  state.gandyFullNameMigration="5.6";
}


function enforceGandyFullNamesV561(){
  var names={
    "Jordan":"Jordan Blake",
    "Maya":"Maya Thompson",
    "Luis":"Luis Ramirez",
  ...
  function upgrade(roster){
    ...
    roster.forEach(function(guard){
      if(!guard||typeof guard.name!=="string")return;
      var current=guard.name.trim();
      if(names[current])guard.name=names[current];
    });
  }
  ...
  state.gandyFullNameMigration="5.6.1-corrected";
}
...
(function(){
  ...
  function report2SystemValues(){
    ...
    var demand=typeof adjustedDemand==="function"?adjustedDemand():0;
  ...
  function bindReportCenter2(){
    ...
    ["report2Type","report2Manager","report2Title"].forEach(function(id){
      r2(id).addEventListener("change",renderReportSystemData);
    });

    managerFieldIds().forEach(function(id){
      r2(id).addEventListener("input",updateReportCompletion);
    });
